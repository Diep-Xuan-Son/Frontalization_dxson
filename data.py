...
class ExternalInputIterator(object):
    ...
    def __init__(self, imageset_dir, batch_size, random_shuffle=False):
        ...
        if self.frontals.size == 0: #them
            # shutil.copyfile(source, destination)
            frontal_list = np.array([[join(person_dir, profile_file) for profile_file in sorted(os.listdir(person_dir)) if is_jpeg(profile_file)] for person_dir in get_subdirs(imageset_dir)])
            self.frontals = frontal_list[0]
            for frontal_unit in frontal_list[1:]:
                self.frontals.append(frontal_unit)
    ...
    def __next__(self):
        ...
        for _ in range(self.batch_size):
            ...
            profile = cv2.imread(profile_filename)
            _, profile = cv2.imencode('.jpg', profile)
            profile = np.array(profile)
            frontal = cv2.imread(frontal_filename)
            _, frontal = cv2.imencode('.jpg', frontal)
            frontal = np.array(frontal)
            profiles.append(profile)
            frontals.append(frontal)
            # print(len(profiles))
...
class ImagePipeline(Pipeline):
    ...
    def __init__(self, imageset_dir, image_size=128, random_shuffle=False, batch_size=64, num_threads=2, device_id=0):
        ...
        self.frontal_ind = eii.frontal_indices  #them
        self.frontal_ids = eii.frontals #them
        ...
        self.norm = ops.CropMirrorNormalize(device="gpu", output_dtype=types.FLOAT,image_type=types.RGB,
                                         mean=[128., 128., 128.], std=[128., 128., 128.],
    ...
    def labels(self):   #them
        return self.frontal_ind
    
    def ids(self):    #them
        return self.frontal_ids
    ...
    def define_graph(self):    
        ...
        # print((profile_output, frontal_output))
