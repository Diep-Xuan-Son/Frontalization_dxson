...
use_pretrain = True
model = torch.load("./output/old/9475folder_200epoch.pt")
epoch_s = 100
...
for epoch in range(epoch_s):
    ...
    if ((epoch+1) % (epoch_s/10)) == 0:
      # Save the inputs, outputs, and ground truth frontals to files:
        ...
        torch.save(netG.cuda().state_dict(),'output/netG_%d.pt' % epoch)
        vutils.save_image(profile.data, 'output/input.jpg', normalize=True)
        vutils.save_image(real.data, 'output/real.jpg', normalize=True)
        vutils.save_image(generated.data, 'output/generated.jpg', normalize=True)
